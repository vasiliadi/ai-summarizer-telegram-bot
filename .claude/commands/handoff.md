...
2. Promote every durable fact this session established — an external-service constraint, a settled choice, a rejected review finding, a convention — into the tracked doc that owns it: `docs/context/architecture.md` for gotchas and standing choices, `docs/context/style-guide.md` for conventions, `AGENTS.md` for process. These are tracked files, so branch off `main` first if you are still on it, or the `PreToolUse` hook blocks the edit. Do this before writing the handoff, so step 4 records it as an output. A handoff is a work log, and agent-local memory is not portable; anything a later session must not get wrong belongs in a tracked file, not only here.
3. Archive next, before writing anything: move every existing `docs/summaries/handoff-*.md` to `docs/archive/`, without overwriting a same-named file already there. Order matters — once the new handoff is on disk it matches that glob too.
4. Write the handoff to `docs/summaries/handoff-[YYYY-MM-DD]-[topic].md`. Never overwrite: if that path still exists after step 3, pick a `[topic]` unique for the date.
