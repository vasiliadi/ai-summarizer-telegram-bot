...
- `docs/context/` — reusable domain knowledge, loaded only when relevant. The durable store *every* agent reads; agent-local memory is not portable and is not a substitute, so anything a later session must not get wrong belongs here. **(tracked)**
