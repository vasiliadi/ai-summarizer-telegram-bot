...
9. Pre-commit hooks (`.pre-commit-config.yaml`) enforce format/lint/type/test checks at commit time — do not run them manually before committing, and never bypass them with `--no-verify`. When the pytest hook runs, review its coverage output — no new uncovered lines. Don't rush to commit: it's fine to keep iterating with uncommitted work; commit when the user asks or a unit of work is genuinely complete (message format: `docs/context/git-guide.md`). **Never `git push` on your own — a push happens only via the user invoking `/create-pr`.** Never stage `docs/summaries/` or `docs/archive/` (gitignored); under `docs/`, only `docs/context/` is tracked.
