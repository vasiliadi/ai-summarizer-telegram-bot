...
9. Before every commit, pass `uvx ruff format .`, `uvx ruff check .`, `uvx ty check .`, and `uv run pytest --cov`, then review the coverage report — no new uncovered lines (full sequence: `docs/context/git-guide.md`). Commit locally when a unit of work is complete. **Never `git push`: a push is authorized only by the user invoking `/create-pr`, and only for the single push that command performs.** Nothing else counts — not an open PR, a review comment, an earlier push or `/create-pr`, not even "push it" in chat; redirect all of these to `/create-pr`. Pushing triggers the GitHub code-review agent, so when a commit is ready, say so and wait. Never stage `docs/summaries/` or `docs/archive/` (gitignored); under `docs/`, only `docs/context/` is tracked.
