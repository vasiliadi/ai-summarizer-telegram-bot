...
**Exception — Alembic migrations:** Ruff formats all project code. `black` is listed in the `dev` dependency group because Alembic invokes it internally to format autogenerated migration files. Do not run `black` manually on project code.
